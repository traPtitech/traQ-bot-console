<template lang="pug">
  q-page.q-pa-md.q-gutter-sm
    ...
    q-form.q-gutter-md(@submit="onSubmit")
      q-input(v-model="id" outlined stack-label label="Webhook ID" :rules="[val => val && /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(val) || 'Webhook IDを入力してください']")
...
export default {
  ...
  methods: {
    async onSubmit () {
      ...
      try {
        await postWebhookMessage(this.id, this.message, this.webhookSecret)
        this.$q.notify({
          icon: 'done',
          color: 'primary',
          textColor: 'white',
